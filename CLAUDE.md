...
- **Use `/converge` for slice hardening.** When a code slice is structurally complete (implementation done, tests passing, ready to commit), suggest `/converge` rather than manually orchestrating `/simplify` and `/review` loops. The skill runs review-first then simplify-second with built-in stopping rules. Don't auto-invoke it on every edit; it's slice-level work, not edit-level.
