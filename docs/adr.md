...
The module targets core 2.9.0-SNAPSHOT (bumped from 2.8 so it compiles against #6084's event classes), so the bridge is no longer a 2.8-compatibility path. It was the incumbent default while the events consumer was being built and verified; now that the consumer is verified end-to-end and `events` is the default (see [Runtime sync-mode selection](#runtime-sync-mode-selection-aop--events-rollout-control)), the bridge is the `aop` opt-out and the resolver-unavailable failure-safe, awaiting retirement once `events` has soaked. The bridge:
...
### Runtime sync-mode selection (AOP → events rollout control)

The bridge and the core-events consumer cannot both fire for the same entity type without double work (correctness is safe — `index()` is idempotent — but each record would be serialized and embedded twice; embedding is the expensive operation). With the module on a 2.9 floor both paths are available in the same deployment, so the switch is a parity / rollout control, not a version shim. A single global property selects the active path:

- **`querystore.syncMode`** ∈ `{aop, events, both}`, default `events` (flipped from `aop` after the consumer was verified end-to-end on a real 2.9 server — save→index and void→delete via the consumer with the bridge gated off).
  - `aop` — the querystore-owned migration bridge (the opt-out, now that `events` is the default; also the resolver-unavailable failure-safe; retired with the bridge per the trajectory below).
...
- **Deprecation trajectory.** This GP is a migration control, not a permanent co-equal-options switch (which would contradict the "AOP is tech debt" stance above). Trajectory: ~~default `aop`~~ → **default `events` now** (flipped after the consumer was built and verified end-to-end) → next: delete the `aop` and `both` modes (and the bridge) together once events has soaked across deployments without regression. `querystore.syncMode` itself is removed when only one path remains.
